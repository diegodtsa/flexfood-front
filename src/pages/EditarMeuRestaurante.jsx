...
const EditarMeuRestaurante = () => {
  ...
  const buscarDados = async () => {
    const response = await axios.get("https://api.dev-diegodtsa.com.br/restaurantes/" + restaurante_id, {
      headers: {
  ...
  const enviarDados = async () => {
    try {
      ...
      const response = await axios.put("https://api.dev-diegodtsa.com.br/restaurantes/" + restaurante_id, formData, {
        headers: {
      ...
      <Container sx={{ mt: 10 }}>
        <Paper>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', px: 5, py: 5 }}>
            ...
            <Box sx={{ mt: 3}}>
              <img src={"https://api.dev-diegodtsa.com.br/" + logoAtual.replace("public", "")}/>
            </Box>
